            ...
            @forelse($interactions as $item)
                <div class="p-4 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition flex items-start space-x-4">
                    ...
                    @if(auth()->user()->isTeamAdmin())
                        <div>
                            <button wire:click="delete('{{ $item->id }}')" wire:confirm="Yakin ingin menghapus log ini?" wire:loading.attr="disabled" wire:target="delete('{{ $item->id }}')" class="text-gray-400 hover:text-red-600 text-sm px-2 py-1 disabled:opacity-50">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl w-full max-w-md p-6 space-y-4 border border-gray-100 dark:border-zinc-700">
            ...
            <form wire:submit.prevent="store" class="space-y-4">
                ...
                <div class="flex justify-end space-x-2 pt-4 border-t dark:border-zinc-700">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="store" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-xs font-medium hover:bg-indigo-700 transition disabled:opacity-50"><span wire:loading.remove wire:target="store">Simpan Log</span><span wire:loading wire:target="store">Menyimpan...</span></button>
                </div>
            </form>
        </div>
    </div>
